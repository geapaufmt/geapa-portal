...
function portalExecutarAcao_(requisicao, runtimeOptions) {
  ...
  if (acao === 'apresentacaoConsultarDecisao') {
    return portalApresentacaoConsultarDecisaoV2(
      requisicao.token || '',
      requisicao.payload || requisicao
    );
  }
...
function portalResposta_(ok, code, message, data, metaExtra) {
  ...
  return {
    ...
    retrySafe: dados.retrySafe === true || meta.retrySafe === true,
    data: dados,
